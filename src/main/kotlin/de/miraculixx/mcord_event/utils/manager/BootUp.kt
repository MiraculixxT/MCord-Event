...
import de.miraculixx.mcord_event.modules.twitch.PointDrop
...
class BootUp(jda: JDA, twitchClient: TwitchClient) {
    ...
    private fun registerEvents(jda: JDA, twitchClient: TwitchClient) {
        ...
        val onlineChecker = PointDrop()
        onlineChecker.onGoOnline(eventManager)
        onlineChecker.onGoOffline(eventManager)
    }
}